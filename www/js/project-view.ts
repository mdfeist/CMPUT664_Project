...
const Timeline = class Timeline {
  ...
  public cellHeight = 64;
  public height: number;

  constructor(private data: DataView, public width: number) {
    this.height = this.cellHeight * data.types.length;
  }

  draw() {
    this.insertLatexSummary();

    const {width, height, data, marginLeft} = this;
  ...
  /**
   * Sneak in a little info about the project.
   */
  insertLatexSummary() {
    d3.select('body').selectAll('.summary')
        .data([summary(this.data)])
        .text(d => d)
      .enter().append('pre')
